...
| `encut` | number, optional | Plane-wave cutoff in eV written to `ENCUT` in every init, relaxation, bilayer/monolayer MD, and validation INCAR. Defaults to `500`. Set it to a value you have already tested, for example `encut: 500`. Each generated POTCAR still contains only the elements in its local POSCAR, in local POSCAR order. |
